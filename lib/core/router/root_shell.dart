...
import '../../features/alerts/providers/price_alert_providers.dart';
...
///
/// Also hosts the price-alert listener (see triggeredAlertsProvider) so
/// an in-app banner can fire from anywhere in the main tabs, not just
/// while a specific stock/crypto detail screen is open.
class RootShell extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen(triggeredAlertsProvider, (previous, next) {
      next.whenData((alerts) {
        for (final alert in alerts) {
          ref.read(priceAlertControllerProvider.notifier).markTriggered(alert.id);
          final scaffoldMessenger = ScaffoldMessenger.maybeOf(context);
          scaffoldMessenger?.showSnackBar(
            SnackBar(
              content: Text(
                alert.direction.name == 'above'
                    ? '🔔 ${alert.symbol} به بالای ${alert.targetPrice.toStringAsFixed(0)} رسید'
                    : '🔔 ${alert.symbol} به زیر ${alert.targetPrice.toStringAsFixed(0)} رسید',
              ),
