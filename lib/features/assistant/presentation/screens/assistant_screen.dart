...
class _AssistantScreenState extends ConsumerState<AssistantScreen> {
  ...
  // Voice input placeholder: the speech_to_text plugin's Android build
  // config conflicted with this project's Gradle/AGP setup and broke CI
  // builds, so it's disabled for now. To re-enable: add speech_to_text
  // back to pubspec.yaml (pick a version compatible with AGP 8.3.0 /
  // compileSdk 34), then wire _toggleListening back to it.
  void _toggleListening() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('ورودی صوتی به‌زودی فعال می‌شود')),
    );
  }
        ...
        child: message.content.isEmpty && message.isStreaming
            ...
            : MarkdownBody(
                ...
                styleSheet: MarkdownStyleSheet(
                  p: TextStyle(color: AppColors.textPrimary),
                ),
              ),
      ),
    );
