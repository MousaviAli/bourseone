...
///
/// Prices are nudged a little on every call (see `_tick`) combined with
/// `marketTickerProvider` polling every few seconds, so the app feels
/// "live" during on-device testing even without a real backend.
class MockMarketRepository implements MarketRepository {
  static final List<_SeedSymbol> _seed = [
    ...
    _SeedSymbol('وتجارت', 'بانک تجارت', 'بانک', 1890),
    _SeedSymbol('کگل', 'گل‌گهر', 'فلزات اساسی', 9760),
    _SeedSymbol('ذوب', 'ذوب‌آهن اصفهان', 'فلزات اساسی', 1640),
  ];

  final Random _rnd = Random();
  late List<StockSymbol> _symbols = _seed.map((s) {
  ...
  /// Nudges every symbol's price a little, simulating a live tick.
  void _tick() {
    _symbols = _symbols.map((s) {
      final drift = (_rnd.nextDouble() - 0.5) * s.lastPrice * 0.006; // ~±0.3%
      final newPrice = (s.lastPrice + drift).clamp(s.lastPrice * 0.7, s.lastPrice * 1.3);
      final newChangePercent = ((newPrice - s.closingPrice) / s.closingPrice * 100);
      return StockSymbol(
        symbol: s.symbol,
        isin: s.isin,
        companyName: s.companyName,
        ...
        changeValue: double.parse((newPrice - s.closingPrice).toStringAsFixed(0)),
        volume: s.volume + _rnd.nextInt(50000),
  ...
  Future<List<MarketIndex>> getIndices() async {
    ...
    final tedpixChange = (_rnd.nextDouble() * 3 - 1);
    final ewChange = (_rnd.nextDouble() * 3 - 1.5);

    return [
      MarketIndex(
        key: 'tedpix',
        title: 'شاخص کل',
        value: (2154870 + _rnd.nextInt(5000)).toDouble(),
        changePercent: double.parse(tedpixChange.toStringAsFixed(2)),
        history: genHistory(2100000),
      ),
      MarketIndex(
        key: 'equalWeight',
        title: 'شاخص هم‌وزن',
        value: (741230 + _rnd.nextInt(2000)).toDouble(),
        changePercent: double.parse(ewChange.toStringAsFixed(2)),
